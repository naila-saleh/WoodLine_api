...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponse> ConfirmEmailAsync(ConfirmEmailRequest request)
    {
        ...
        // Token was Base64Url-encoded when the link was built â€” decode it back to standard Base64
    ...
    public async Task<AuthResponse> RefreshTokenAsync(string refreshToken)
    {
        ...
        {
            return Fail("Refresh token cookie is missing.");
        }

        var existingRefreshToken = await _authRepository.GetRefreshTokenAsync(refreshToken);
        if (existingRefreshToken is null || !existingRefreshToken.IsActive)
        {
            return Fail("Invalid or expired refresh token.");
        }

        var user = await _authRepository.FindByIdAsync(existingRefreshToken.UserId);
        if (user is null)
        {
            return Fail("User not found.");
        }
    ...
    private async Task SendConfirmationEmailAsync(ApplicationUser user, HttpRequest requestHttp)
    {
        try
        {
            ...
            // Convert standard Base64 â†’ Base64Url (URL-safe: no +, /, or = chars)
            ...
            var emailBody = $@"
                ...
                <body>
                    <div class='container'>
                        <div class='header'>
                            <h2>Email Confirmation - WoodLine</h2>
                        </div>
                        <div class='content'>
                        ...
                        <div class='footer'>
                            ...
                            <p>&copy; 2026 WoodLine. All rights reserved.</p>
                        </div>
                    </div>
                </body>
                </html>
            ...
            await _emailSender.SendEmailAsync(user.Email, "Confirm your WoodLine email address", emailBody);
            _logger.LogInformation($"[EMAIL] âœ“ Email sent successfully to {user.Email}");
        }
        catch (Exception ex)
        {
